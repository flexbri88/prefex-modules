...
class Call_control extends CI_Controller
{
    ...
    public function handleCall()
    {
        ...
        $isConference = isset($_GET['conference']) ? $_GET['conference'] : null;
        if (isset($callerIdNumber)) {
            if (get_option('call_twilio_recording_active')) {
                ...
                if (isset($phoneNumberToDial)) {
                    ...
                    if ($staff->ongoing_call && 2 != $staff->ongoing_call) {
                        $dial->client('support_agent_' . $staff_id);
                    } else {
                        $staff_id = $this->getNotBusyStaff();
                        if ($staff_id) {
                            $dial->client('support_agent_' . $staff_id);
                        } else {
                            ...
                            // $call = $twilio->calls($_GET['CallSid'])->fetch();
                            // print_r($call);
                            // die;
                            $response->say('Please wait, all agents are busy on call...', ['loop' => 1, 'voice' => 'woman']);
                            $response->enqueue('all_agents_busy', ['action' => base_url('lead_manager/Call_control/holdIncomingCall'), 'method' => 'GET']);
                        }
                    }
                }
                if ($isConference) {
    ...
    public function getNotBusyStaff()
    {
        if (!class_exists('lead_manager_model', false)) {
            $this->load->model('lead_manager_model');
        }
        $staff = $this->lead_manager_model->get_rand_active_staff();
        return isset($staff) && !empty($staff) ? $staff['staffid'] : NULL;
    }
    public function getNotBusyStaffRecursive()
    {
        if (!class_exists('lead_manager_model', false)) {
            $this->load->model('lead_manager_model');
        }
        $staff = $this->lead_manager_model->get_rand_active_staff();
        return isset($staff) && !empty($staff) ? $staff['staffid'] : $this->getNotBusyStaffRecursive();
    }
    public function holdIncomingCall()
    {
        $staff_id = $this->getNotBusyStaffRecursive();
        if ($staff_id) {
            $response = new VoiceResponse();
            $dial = $response->dial('');
            $dial->client('support_agent_' . $staff_id);
            $dial->queue('all_agents_busy', ['url' => base_url('lead_manager/Call_control/connectWithActiveAgent/' . $staff_id), 'method' => 'GET']);
            header('Content-Type: text/xml');
            echo $response;
        }
    }
    public function connectWithActiveAgent($staff_id)
    {
        $response = new VoiceResponse();
        $dial = $response->dial('');
        $dial->client('support_agent_' . $staff_id);
